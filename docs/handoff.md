...
| 上方收款季度     | 依**收款月份**；預設＝今天所在財務季（**8 月開站→Q3，正確**）；列表／總覽／匯出皆同                              |
| 發放篩選總覽     | 跟上方篩選；含區間內回簽月份筆數                                                                                |
| 上季獎金發放     | 與上方篩選**獨立**：公司發放月 5／8／11／2 → 統計對應收款季（8 月發＝Q2 的 5–7 月收款）；可切換波次              |
...
## 9. 個人版驗收評分（2026-07-01）

> **適用範圍**：過渡方案（單一共用帳號 + RLS），**不含** spec §2–§5 正式多帳號版。  
> **目的**：判斷能否當日常主力帳本；非正式版 roadmap。

### 9.1 評分標準（100 分）

| #   | 維度           | 權重 | 評什麼                                       |
| --- | -------------- | ---- | -------------------------------------------- |
| A   | 核心帳本功能   | 25   | 抓報價、編輯、倍率、試算、匯出、再同步       |
| B   | 找案效率       | 15   | 季度篩選、模糊搜尋、KPI 總覽                 |
| C   | 手機可用性     | 15   | 卡片、側欄 drawer、輸入、閃爍／捲軸         |
| D   | 桌面可用性     | 10   | 寬表、sticky 欄、操作流程                    |
| E   | 資料可靠       | 15   | Supabase、RLS、重複 URL 更新、錯誤重試     |
| F   | 安全（個人版） | 10   | 密碼閘、改密、限流、session                  |
| G   | 工程維護       | 10   | CI、單元測試、模組拆分、文件                 |

**等級對照**

| 分數  | 等級 | 意味                     |
| ----- | ---- | ------------------------ |
| 90+   | A    | 日常主力，僅剩小 polish    |
| 80–89 | B    | 可安心自用（**目前區間**） |
| 70–79 | C    | 能用但常卡               |
| <70   | D    | 不建議當主力             |
...
| 維度 | 得分 /10 | 加權 | 備註                                             |
| ---- | -------- | ---- | ------------------------------------------------ |
| A    | 9.0      | 22.5 | 主流程完整；無還原／備份還原                       |
| B    | 9.0      | 13.5 | 搜尋 + 高亮 + CSV 檔名；40+ 筆夠用               |
| C    | 8.5      | 12.8 | 卡片／側欄成熟；極窄螢幕 KPI 仍略擠              |
| D    | 9.0      | 9.0  | 桌面表格成熟；欄多需橫捲（可接受）               |
| E    | 8.5      | 12.8 | Supabase 穩；quote 站若改 SPA 正式站可能抓不到   |
| F    | 8.0      | 8.0  | 共用密碼可接受；非多使用者隔離                   |
| G    | 8.5      | 8.5  | CI + Vitest + E2E smoke；無完整抓報價 E2E       |
|      |          | **87** | 四捨五入                                         |
...
- [ ] Vercel / Supabase 環境變數正常（能登入、能抓報價）
- [ ] `gate@ivy.app` 密碼足夠強；必要時用側欄「修改密碼」

**建議（低頻）**

- [ ] 每季匯出 CSV 本地備份（檔名已含季度 + 時間）
- [ ] 偶爾用手機抽測卡片／搜尋

**不必做（個人版）**

- Vercel 設 Playwright（程式預設 `PLAYWRIGHT_FALLBACK=0`）
- 多帳號、`/admin`、業務隔離（屬 spec 正式版）
- 本機 Playwright（除非 httpx 抓不到報價）

**已知風險（出問題再處理）**

| 風險               | 緩解                                       |
| ------------------ | ------------------------------------------ |
| quote 站改動態渲染 | 本機 `.env.local` 設 `PLAYWRIGHT_FALLBACK=1` |
| 共用密碼外洩       | 定期改密；未來若要做方案 B 再換多帳號      |
| 誤刪資料           | 定期 CSV 備份                              |

### 9.4 可選加分項（衝 90+，非急）

- 極窄手機 KPI 字級再縮
- 搜尋關鍵字 localStorage 記憶
- 季度末備份提醒
- 報價抓取健康監控（quote 站改版後）

---

_最後更新：2026-07-01（§9 個人版驗收評分 87/100）_
